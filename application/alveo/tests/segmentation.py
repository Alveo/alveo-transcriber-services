...
class AlveoSegmentationTests(AlveoTests):
    ...
    def testSegmentationAuthNoDoc(self):
        response, status = self.get_json_response('/segment?remote_url='+self.ALVEO_API_URL, self.DEFAULT_HEADERS)
    ...
    def testSegmentationValidDocument(self):
        ...
        response, status = self.get_json_response('/segment?remote_url='+remote_url, self.DEFAULT_HEADERS)
