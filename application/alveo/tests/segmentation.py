...
class AlveoSegmentationTests(AlveoTests):
    ...
    def testSegmentationAuthNoDoc(self):
        response, status = self.get_json_response(DOMAIN+'/segment?remote_url='+self.ALVEO_API_URL, self.DEFAULT_HEADERS)
    ...
    def testSegmentationInvalidDocument(self):
        ...
        response, status = self.get_json_response(DOMAIN+'/segment?remote_url='+invalid_remote_url, self.DEFAULT_HEADERS)
    ...
    def testSegmentationValidDocument(self):
        ...
        response, status = self.get_json_response(DOMAIN+'/segment?remote_url='+remote_url, self.DEFAULT_HEADERS)
